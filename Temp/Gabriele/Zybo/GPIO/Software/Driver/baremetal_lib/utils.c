...
int32_t gic_disable(uint32_t GIC_ID){
    return Xil_ExceptionDisable();
}

uint32_t *gic_register_interrupt(XScuGic gic_inst ,uint32_t interrupt_line, void* interrupt_handler){

    int status = XScuGic_Connect(&gic_inst, interrupt_line, (Xil_InterruptHandler)interrupt_handler, ( void*) &gic_inst);
	if (status != XST_SUCCESS){
			return status;
		}
...
uint32_t *gic_register_interrupt_handler(XScuGic gic_inst, interrupt_handler* interrupt_handler){
    return gic_register_interrupt(XScuGic gic_inst , interrupt_handler->interrupt_line, interrupt_handler->interrupt_handler){
